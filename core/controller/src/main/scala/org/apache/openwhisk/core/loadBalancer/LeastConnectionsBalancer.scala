...
/**
 * A loadbalancer that schedules workload based on least-connections algorithm.
 */
...
class LeastConnectionsBalancer(
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val chosen = if (invokersToUse.nonEmpty) {
      val invoker: Option[(InvokerInstanceId, Boolean)] = LeastConnectionsBalancer.schedule(
...
object LeastConnectionsBalancer extends LoadBalancerProvider {
  ...
  def schedule(
    ...
    if (numInvokers > 0) {
      var maxAvailablePermits = dispatched(0).availablePermits
      var invokerToChoose: Int = 0
      
      for (i <- 1 until numInvokers) {
        val currentAvailablePermits = dispatched(i).availablePermits
        if (currentAvailablePermits > maxAvailablePermits) {
          maxAvailablePermits = currentAvailablePermits
          invokerToChoose = i
        }
      }
